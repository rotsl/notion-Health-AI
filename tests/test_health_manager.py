...
class TestHealthManager:
    ...
    @pytest.mark.asyncio
    async def test_build_tribe_text_context(
        self, health_manager, mock_notion_client, mock_health_metrics
    ):
        """Test building a TRIBEv2 text context from connected Notion data."""
        health_manager.client = mock_notion_client
        mock_notion_client.query_health_metrics = AsyncMock(return_value=mock_health_metrics)
        mock_notion_client.query_symptoms = AsyncMock(
            return_value=[
                SymptomLog(
                    symptom_date=date.today(),
                    symptom_name="Headache",
                    severity=SymptomSeverity(6),
                    body_part="Head",
                )
            ]
        )
        mock_notion_client.query_goals = AsyncMock(
            return_value=[
                HealthGoal(
                    title="Sleep 8 hours",
        ...
        context = await health_manager.build_tribe_text_context(context_days=7)

        assert context is not None
        assert "average sleep" in context
        assert "recent symptoms" in context
        assert "active goals" in context

    @pytest.mark.asyncio
    async def test_run_brain_analysis_uses_connected_context(
        self, health_manager, mock_notion_client
    ):
        """Test that brain analysis passes Notion context into the TRIBEv2 path."""
        health_manager.client = mock_notion_client
        health_manager.build_tribe_text_context = AsyncMock(return_value="recent health context")
        health_manager._get_tribe = AsyncMock()
        tribe = health_manager._get_tribe.return_value
        tribe.model_wrapper.is_loaded = False
        tribe.predict_brain_response = AsyncMock(
            return_value={
                "success": True,
                "prediction": {
                    "activity": "exercise",
                    "duration_minutes": 30,
                    "brain_regions": {
                        "motor_cortex": {"response": "excitatory", "intensity": 0.8},
                        "prefrontal_cortex": {"response": "excitatory", "intensity": 0.7},
                        "anterior_cingulate": {"response": "excitatory", "intensity": 0.6},
                    },
                    "wellness_score": 8.1,
        ...
        mock_notion_client.create_brain_analysis = AsyncMock(return_value="brain_page_id")
        health_manager.brain_database_id = "brain_db"

        result = await health_manager.run_brain_analysis("exercise", 30, context_days=7)

        assert result.success is True
        tribe.predict_brain_response.assert_awaited_once_with(
            "exercise",
            30,
            health_context="recent health context",
        )
        assert result.data["health_context_used"] is True
