...
class TestAIInsightsEngine:
    ...
    @pytest.mark.asyncio
    async def test_generate_health_insights_includes_structured_brain_context(
        self, engine, mock_health_metrics, mock_health_summary
    ):
        """Structured details should carry brain context for the GUI renderer."""
        engine.client = None
        insights = await engine.generate_health_insights(
            mock_health_metrics,
            mock_health_summary,
            ...
            brain_context={
                ...
                "duration_minutes": 25,
                "wellness_score": 0.82,
                "top_regions": ["prefrontal_cortex", "hippocampus"],
            },
        )

        assert len(insights) == 1
        insight = insights[0]
        assert insight.structured_details is not None
        assert "brain_context" in insight.structured_details
...
class TestTribeHealthAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_predict_brain_response_uses_health_context(self, analyzer):
        """Test that health context is fed into the text stimulus path."""
        analyzer.model_wrapper.predict_response = AsyncMock(
            return_value={
                "motor_cortex": {"activation": 0.8, "type": "excitatory"},
                "amygdala": {"activation": 0.2, "type": "inhibitory"},
                "prefrontal_cortex": {"activation": 0.7, "type": "excitatory"},
            }
        )

        result = await analyzer.predict_brain_response(
            "exercise",
            duration_minutes=30,
            health_context="average sleep 7.2 hours",
        )

        assert result["success"] is True
        analyzer.model_wrapper.predict_response.assert_awaited_once()
        text_stimulus = analyzer.model_wrapper.predict_response.await_args.kwargs["text_stimulus"]
        assert "average sleep 7.2 hours" in text_stimulus
        assert result["prediction"]["health_context_used"] is True
