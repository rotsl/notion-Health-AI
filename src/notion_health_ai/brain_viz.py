...
try:
    ...
    from nilearn import surface as _nl_surface
...
class BrainVisualizer:
    ...
    @staticmethod
    def _align_timesteps(preds: np.ndarray, duration_minutes: Optional[int]) -> np.ndarray:
        """Resample multi-step predictions so each timestep maps to one simulated minute."""
        if preds.ndim != 2 or not duration_minutes or duration_minutes <= 0:
            return preds

        n_current, n_vertices = preds.shape
        if n_current == duration_minutes:
            return preds

        x_old = np.linspace(0.0, 1.0, n_current)
        x_new = np.linspace(0.0, 1.0, duration_minutes)
        aligned = np.empty((duration_minutes, n_vertices), dtype=np.float32)
        for idx in range(n_vertices):
            aligned[:, idx] = np.interp(x_new, x_old, preds[:, idx])
        return aligned

    @staticmethod
    def _time_labels(n_timesteps: int, duration_minutes: Optional[int]) -> List[str]:
        """Build readable time labels for sliders and animation frames."""
        if not duration_minutes or duration_minutes <= 0:
            return [f"{t}s" for t in range(n_timesteps)]
        if n_timesteps == 1:
            return [f"{duration_minutes} min"]

        minute_marks = np.linspace(1, duration_minutes, n_timesteps)
        labels = []
        for minute_mark in minute_marks:
            minutes = int(round(float(minute_mark)))
            if minutes >= 60 and minutes % 60 == 0:
                hours = minutes // 60
                labels.append(f"{hours} hr" if hours == 1 else f"{hours} hrs")
            else:
    ...
    def plot_interactive(
        ...
        if preds.ndim == 1:
            ...
            preds_2d = preds
        preds_2d = self._align_timesteps(preds_2d, duration_minutes)
        n_timesteps, n_verts = preds_2d.shape
        time_labels = self._time_labels(n_timesteps, duration_minutes)
        ...
        if n_timesteps == 1:
            ...
            layout_base["updatemenus"] = [
                dict(
                    ...
                    font=dict(color="#08111f", size=14),
                )
            ]
            layout_base["sliders"] = [
                dict(
        ...
        if n_timesteps > 1:
            fig.add_annotation(
                text=(
                    f"TRIBEv2 · {n_timesteps} timesteps · "
                    f"{duration_minutes or n_timesteps} simulated minutes · "
                    f"{n_verts} cortical vertices"
                ),
                xref="paper",
                yref="paper",
                x=0.5,
    ...
    def plot_gif(
        ...
        preds = self._align_timesteps(preds, duration_minutes)
        time_labels = self._time_labels(preds.shape[0], duration_minutes)
        ...
        for t in range(n_frames):
            ...
            fig.suptitle(f"{title}  [{time_labels[t]}]", fontsize=11)
        ...
        def _render_frame(t: int):
            ...
            for ax, (mesh, data, hemi, lbl) in zip(
                ...
                try:
                    ...
                    pass
                ax.set_title(f"{lbl}  {time_labels[t]}", fontsize=9)
