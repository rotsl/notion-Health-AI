...
class HealthManager:
    ...
    async def build_tribe_text_context(self, context_days: int = 7) -> Optional[str]:
        """
        Build a compact text summary from connected Notion data for TRIBEv2 text prompts.

        Args:
            context_days: How many recent days of Notion data to summarize

        Returns:
            Short text summary or None if no useful context is available
        """
        if context_days <= 0:
            return None
        ...
        try:
            if self.health_database_id:
                ...
                if metrics:
                    ...
                    metric_bits = [f"recent health data from the past {context_days} days"]
                    if summary.avg_sleep_hours is not None:
                        metric_bits.append(f"average sleep {summary.avg_sleep_hours:.1f} hours")
                    if summary.avg_mood is not None:
                        metric_bits.append(f"average mood {summary.avg_mood:.1f}/10")
                    if summary.avg_energy is not None:
                        metric_bits.append(f"average energy {summary.avg_energy:.1f}/10")
                    if summary.total_exercise_minutes is not None:
                        metric_bits.append(
                            f"total exercise {int(summary.total_exercise_minutes)} minutes"
                        )
                    if summary.avg_daily_steps is not None:
                        metric_bits.append(f"average steps {int(summary.avg_daily_steps)}")
                    if summary.weight_change is not None:
                        metric_bits.append(f"weight change {summary.weight_change:+.1f} lbs")
                    context_parts.append(", ".join(metric_bits) + ".")
        except Exception as exc:
            logger.warning(f"Failed to summarize health metrics for TRIBEv2 context: {exc}")

        try:
            if self.symptoms_database_id:
                symptoms = await self.get_symptom_history(start_date, date.today())
                if symptoms:
                    recent_symptoms = sorted(
                        symptoms,
                        key=lambda s: (s.symptom_date, s.symptom_time or 0),
                        reverse=True,
                    )[:3]
                    symptom_bits = [
                        f"{symptom.symptom_name} severity {symptom.severity.value}"
                        for symptom in recent_symptoms
                    ]
                    context_parts.append("recent symptoms: " + "; ".join(symptom_bits) + ".")
        except Exception as exc:
            logger.warning(f"Failed to summarize symptoms for TRIBEv2 context: {exc}")

        try:
            if self.goals_database_id:
                goals = await self.client.query_goals(self.goals_database_id, active_only=True)
                if goals:
                    goal_bits = [goal.title for goal in goals[:3]]
                    context_parts.append("active goals: " + "; ".join(goal_bits) + ".")
        except Exception as exc:
            logger.warning(f"Failed to summarize goals for TRIBEv2 context: {exc}")
    ...
    async def run_brain_analysis(
        ...
        try:
            ...
            health_context = await self.build_tribe_text_context(context_days)

            # Predict brain response for the activity
            prediction = await tribe.predict_brain_response(
                activity,
                duration_minutes,
            ...
            session = BrainAnalysisSession(
                ...
                health_context=health_context
                or f"Activity: {activity}, Duration: {duration_minutes} min",
