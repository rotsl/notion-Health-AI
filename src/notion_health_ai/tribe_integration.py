...
def _import_torch():
    """Import torch only when the real model path is actually needed."""
    return importlib.import_module("torch")


def _get_tribe_model_class():
    """Resolve TribeModel lazily to avoid importing torch during module import."""
    global _TribeModelClass
    if _TribeModelClass is None:
        _TribeModelClass = importlib.import_module("tribev2").TribeModel
    return _TribeModelClass


def _slugify_label(value: str) -> str:
    """Create stable, readable filename fragments."""
    slug = re.sub(r"[^a-z0-9]+", "_", value.lower()).strip("_")
    return slug[:80] or "brain_activation"
...
class TribeModelWrapper:
    ...
    def __init__(self, device: Optional[str] = None):
        ...
        """
        # Default to CPU so importing the app does not require torch to initialise.
    ...
    def _get_device(self, torch_module: Any) -> str:
        """Determine the best available device once torch is imported."""
    ...
    async def load_model(self, cache_dir: str = "./cache") -> bool:
        ...
        try:
            ...
            tribe_model_class = _get_tribe_model_class()
            ...
            if not hf_token:
                logger.error(
                    "No HuggingFace token found. Set HUGGING_FACE_TOKEN to a token with "
                    "read access to facebook/tribev2. The model is gated and cannot be "
                    "downloaded without authentication. Falling back to simulated predictions."
                )
                return False
            ...
            try:
                ...
                logger.error(
                    ...
                    "Verify HUGGING_FACE_TOKEN is valid and you have accepted the model "
                    "license at https://huggingface.co/facebook/tribev2. "
                    "Falling back to simulated predictions."
                )
                return False

            # ------------------------------------------------------------------
            # Step 3: Load the model, passing token explicitly when supported.
            # ------------------------------------------------------------------
            ...
            feature_device = "cuda" if torch_module.cuda.is_available() else "cpu"
            ...
            try:
                if "token" in inspect.signature(tribe_model_class.from_pretrained).parameters:
            ...
            self.model = tribe_model_class.from_pretrained(self.MODEL_NAME, **load_kwargs)
            ...
            logger.error(
                ...
                "If this is a 401/403 error, ensure HUGGING_FACE_TOKEN is valid "
                "and you have accepted the license at https://huggingface.co/facebook/tribev2."
            )
            return False
...
class TribeHealthAnalyzer:
    ...
    async def predict_brain_response(
        ...
        Args:
            ...
            health_context: Optional summary from connected health data
        ...
        text_stimulus = (
            f"Activity: {activity_type.value}. Duration: {duration_minutes} minutes. "
            f"Expected focus: {', '.join(pattern['benefits'][:2])}."
        )
        if health_context:
            text_stimulus += f" Personal health context: {health_context}"

        roi_dict = await self.model_wrapper.predict_response(text_stimulus=text_stimulus)
        roi_brain_responses = self._roi_dict_to_brain_responses(roi_dict)

        brain_responses = []
        for region_name, values in roi_brain_responses.items():
            try:
            ...
            brain_responses.append(
                BrainResponse(
                    region=region,
                    activation_level=float(values.get("activation_level", 0.5)),
                    response_type=values.get("response_type", "neutral"),
                    confidence=float(values.get("confidence", 0.65)),
                    description=values.get("description", self.region_descriptions.get(region, "")),
                )
            )

        avg_activation = (
            sum(response.activation_level for response in brain_responses) / len(brain_responses)
            if brain_responses
            else 0.5
        )
        ...
        wellness_score = min(
            10.0,
            round(base_score * 0.65 + avg_activation * 3.0 + duration_bonus, 2),
        )
    ...
    async def predict_brain_multimodal(
        ...
        label = output_label or f"multimodal {' + '.join(modalities_used)}"
        if duration_minutes:
            title = f"TRIBEv2 Multimodal Simulation: {label} ({duration_minutes} min)"
        else:
            title = f"TRIBEv2 Multimodal Analysis: {label}"
        stem = f"multimodal_{_slugify_label(label)}"
        flat_roi = {k: v.get("activation_level", 0.5) for k, v in brain_responses.items()}
