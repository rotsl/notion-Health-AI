...
class HealthMetric(BaseModel):
    ...
    exercise_calories: Optional[int] = Field(None, ge=0, description="Calories burned during exercise")
...
class Medication(BaseModel):
    ...
    custom_frequency: Optional[str] = Field(None, max_length=100, description="Custom frequency description")
    ...
    prescribing_doctor: Optional[str] = Field(None, max_length=200, description="Doctor who prescribed")
    pharmacy: Optional[str] = Field(None, max_length=200, description="Pharmacy name")
    prescription_number: Optional[str] = Field(None, max_length=50, description="Prescription/Rx number")
    ...
    side_effects: Optional[List[str]] = Field(default_factory=list, description="Known side effects")
    notes: Optional[str] = Field(None, max_length=2000, description="Additional notes")
...
class Appointment(BaseModel):
    ...
    doctor_name: str = Field(..., min_length=1, max_length=200, description="Doctor or provider name")
    ...
    preparation_notes: Optional[str] = Field(None, max_length=2000, description="Preparation instructions")
    questions_to_ask: Optional[List[str]] = Field(default_factory=list, description="Questions for doctor")
...
class SymptomLog(BaseModel):
    ...
    duration_description: Optional[str] = Field(None, max_length=100, description="Duration description")
    ...
    medications_taken: Optional[List[str]] = Field(default_factory=list, description="Medications taken")
    impact_on_daily: Optional[int] = Field(None, ge=1, le=10, description="Impact on daily life 1-10")
    mood_at_time: Optional[MoodLevel] = Field(None, description="Mood when symptom occurred")
    associated_symptoms: Optional[List[str]] = Field(default_factory=list, description="Related symptoms")
    notes: Optional[str] = Field(None, max_length=2000, description="Additional notes")
    created_at: datetime = Field(default_factory=datetime.now)
    updated_at: datetime = Field(default_factory=datetime.now)
...
class BrainAnalysisSession(BaseModel):
    ...
    input_type: str = Field("health_narrative", description="Input used: health_narrative, symptom_text, activity")
    input_summary: str = Field(..., description="Brief description of the health data analysed")

    # TRIBEv2 neural prediction metrics
    mean_activation: Optional[float] = Field(None, description="Mean brain activation across ~20k vertices")
    activation_variance: Optional[float] = Field(None, description="Variance in activation")
    cognitive_load_score: Optional[float] = Field(None, ge=0, le=10, description="Estimated cognitive load 0-10")
    stress_indicator: Optional[float] = Field(None, ge=0, le=10, description="Estimated stress level 0-10")
    emotional_valence: Optional[float] = Field(None, ge=-1, le=1, description="Emotional tone: -1 (negative) to +1 (positive)")

    # Brain network activations
    default_mode_activity: Optional[float] = Field(None, ge=0, le=1, description="Default mode network 0-1")
    executive_control_activity: Optional[float] = Field(None, ge=0, le=1, description="Executive control network 0-1")
    salience_network_activity: Optional[float] = Field(None, ge=0, le=1, description="Salience/stress detection network 0-1")
    ...
    model_available: bool = Field(False, description="True if TRIBEv2 was directly run, False if AI fallback used")
    created_at: datetime = Field(default_factory=datetime.now)
...
class CognitiveInsight(BaseModel):
    ...
    avg_cognitive_load: Optional[float] = Field(None, description="Mean cognitive load across sessions")
    avg_stress_indicator: Optional[float] = Field(None, description="Mean stress level across sessions")
    ...
    sleep_cognition_correlation: Optional[float] = Field(None, description="Sleep quality vs cognitive load")
    mood_brain_correlation: Optional[float] = Field(None, description="Mood vs emotional valence")
    exercise_cognition_correlation: Optional[float] = Field(None, description="Exercise vs cognitive load")
