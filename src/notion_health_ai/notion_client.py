...
class NotionClient:
    ...
    async def create_health_metric(self, database_id: str, metric: HealthMetric) -> str:
        ...
        """
        properties = {
            "Date": {"date": {"start": metric.date.isoformat()}},
        }
    ...
    async def create_appointment(self, database_id: str, appointment: Appointment) -> str:
        ...
        properties = {
            "Date": {"date": {"start": appointment.date.isoformat()}},
        ...
        if appointment.time:
            properties["Time"] = {
                "rich_text": [{"text": {"content": appointment.time.strftime("%H:%M")}}]
            }
    ...
    def _parse_appointment(self, page: Dict) -> Optional[Appointment]:
        ...
        try:
            ...
            return Appointment(
                id=page["id"],
                date=get_date_value("Date"),
                time=time_obj or datetime.now().time(),
    ...
    def _parse_symptom(self, page: Dict) -> Optional[SymptomLog]:
        ...
        try:
            ...
            return SymptomLog(
                ...
                duration_minutes=get_number("Duration (min)"),
                notes=get_text("Notes"),
            )
